...
export default function MapBoardPage(): JSX.Element {
  ...
  const lastPage = Math.ceil((dataBoardsCount?.fetchBoardsCount ?? 10) / 10); // dataBoardsCount.fetchBoardsCount가 undefined이면 10으로 해라.
  // data가 바뀌게되면 다시실행되어 다시 그림..
  console.log(data);
  ...
  return (
    <div>
      ...
      {new Array(10).fill(1).map(
        (_, index) =>
          index + startPage <= lastPage && ( // 얘일때만 보여줘
            <span
              key={index + startPage}
